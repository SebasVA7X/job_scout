"""
scraper/iom_scraper.py
Scrapes IOM (International Organization for Migration) jobs via Oracle HCM REST API.
Endpoint discovered via Network tab inspection.

Portal: https://fa-evlj-saasfaprod1.fa.ocs.oraclecloud.com/hcmUI/CandidateExperience/en/sites/CX_1001/jobs
...
JOB_URL   = f"{BASE_URL}/hcmUI/CandidateExperience/en/sites/CX_1001/requisitions/preview/{{job_id}}"
PORTAL    = f"{BASE_URL}/hcmUI/CandidateExperience/en/sites/CX_1001/jobs"
    ...
    def fetch(self) -> List[Dict[str, Any]]:
        ...
        while True:
            params = {
                ...
                "finder":   f"findReqs;siteNumber=CX_1001,limit={PAGE_SIZE},offset={offset},sortBy=POSTING_DATES_DESC",
            }
            ...
            offset += PAGE_SIZE
            logger.info(f"[IOM] offset={offset}: {len(new)} kept (total: {offset}/{total})")
            ...
            if oldest_date and oldest_date < cutoff:
                logger.info(f"[IOM] Reached cutoff ({cutoff.date()}), stopping")
                break

        logger.info(f"[IOM] Total jobs after filters: {len(jobs)}")
        return jobs
